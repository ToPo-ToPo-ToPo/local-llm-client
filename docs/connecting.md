...
### 共有設定ファイル（複数エージェントで集中管理）

エージェントが多く、1つ1つに `base_url` / `api_key` を書きたくない場合は、**`local-llm-client.toml`**
を置くと、クライアントが**カレントディレクトリから親を遡って**最初に見つかったものを既定として読む
（`.git` / `.env` と同じ流儀）。エージェント側のコード変更は不要。

```toml
# 例: ~/my_programs/01_github/local-llm-client.toml （この下の全エージェントが自動で使う）
api_key  = "＜キー＞"
base_url = "http://192.168.1.5:8799/v1"   # 任意
```

- **ワークスペース root に 1 つ**置けば、その配下の全エージェントが拾う（home 直下は使わない）。
- 特定プロジェクト直下に置けば**そこが優先**（近いディレクトリが勝つ）。
- 優先順位は **明示引数 `LLMClient(api_key=...)` ＞ 設定ファイル ＞ 既定**。
- ファイルが無ければ従来どおり（ローカル・認証なし）。秘密情報なので、リポジトリ内に置くなら
  `.gitignore` に追加するか、リポジトリ外（ワークスペース root）に置く。
